$(document).ready(function(){
   ...
   * Add a Session
   */
  $("#content").on("click", "#create-session", function() {
    if (editingGame == 1) {
      return;
    }
    var bowlerId = $(this).attr("bowler");
    var post_url = "addsession";
   ...
   * Select Bowler
   */
  $("#bowler-select").change(function() {
    $("#select-bowler").submit();
  });
   ...
   * Add a Game
   */
  $("#content").on("click", ".add-game", function() {
    if (editingGame == 1) {
      return;
    }
    var sessionId = $(this).attr("sessionId");
  ...
  $("body").click(function() {
    $(".add-bowler-to-game > ul").animate({height:"hide"},200);
  });
